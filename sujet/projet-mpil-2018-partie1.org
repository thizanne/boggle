...
L'archive que vous avez téléchargée contient le squelette d'une
...
l'environnement de la PPTI *ne seront pas corrigés*. Il est inutile de
...
base accessible à un nouvel utilisateur). L'exigence est la même que
pour OCaml : les projets qui ne compilent pas ou ne s'exécutent pas
avec ces commandes ne seront pas corrigés.
...
Les itérateurs sont semblables aux /streams/ que vous avez déjà vu,
mais ils sont purement fonctionnels : parcourir un itérateur en le
forçant ne le consume pas, il reste utilisable. Ainsi on peut obtenir
...
tableaux. Pour écrire ~from_string~, vous aurez besoin de calculer la
dimension de la grille à générer. Vous pouvez pour cela utiliser la
fonction ~sqrt : float -> float~ qui calcule la racine carrée d'un
nombre (flottant !).
...
- Deux opérations du programme peuvent échouer (des fonctions liées
  ...
  programme OCaml, vous pourrez écrire ~exit 1~.
