...
/Boggle/ [fn:boggle] est un jeu de lettres où les joueurs doivent, en temps
...
Cet énoncé comporte \pageref{before-appendices} pages
(\pageref{LastPage} avec les Annexes). Ne soyez pas intimidé ou
...
La Section [[sec:consignes-generales]] donne les consignes générales
à suivre pour réaliser ce projet. La Section [[sec:enonce-detaille]]
détaille l'énoncé et précise le travail à réaliser.
...
dans le projet. La Section [[sec:introduction-modules]] constitue un
rappel du fonctionnement de la programmation modulaire en OCaml. La
Section [[sec:interfaces]] indique les interfaces des différents modules
à implémenter (cf. Section [[sec:enonce-detaille]]). Vous n'êtes pas
obligés de lire ces annexes pour réaliser le projet, mais elles
peuvent le cas échéant vous apporter des informations utiles.
...
cet énoncé (les différents répertoires comme l'organisation des
modules).
...
l'archive et dézippez la (=$ tar xzvf boggle.tar.gz= sous Linux). Elle
...
Si vous avez déjà une ligne similaire, remplacez la par celle-ci. En
cas de doute, envoyez un mail
...
l'exécute. Ces deux commandes fonctionnent par défaut dans
l'environnement OCaml de la PPTI.

Les rendus qui ne *compilent pas* avec ces commandes dans
l'environnement de la PPTI (avec la configuration précédemment
indiquée) *ne seront pas corrigés*. Il est inutile de rendre une
fonction qui ne compile même pas en espérant qu'elle soit correcte :
pour des raisons évidentes, elle n'a aucune chance de l'être...

Le squelette OCaml contient une version préécrite de ces commandes. Si
vous utilisez un autre langage, vous devrez écrire un =Makefile= et un
script =run.sh= qui respectivement compile et exécute le projet dans
l'environnement par défaut de la PPTI (c'est-à-dire l'environnement de
base accessible à un nouvel utilisateur, sans configuration
supplémentaire). L'exigence est la même que pour OCaml : les projets
qui ne compilent pas ou ne s'exécutent pas avec ces commandes ne
seront pas corrigés.
...
*Les rendus ne respectant pas strictement ces consignes ne seront pas
corrigés !* Assurez-vous de les avoir comprises et n'attendez pas la
dernière minute pour manquer le rendu à cause de la précipitation...
...
exporte. Toutes les fonctions qui ne forcent par l'itérateur (voir la
...
tableaux de caractères : ~char array array~. Les fonctions
à implémenter ne devraient pas poser de problème particulier.
...
représentation d'un lexique par son nœud racine. Le champ ~eow~
indique le cas échéant que ce nœud correspond à une fin de mot (le
lexique ayant cette racine contient donc le mot vide ~""~), et le
champ ~words~ représente les arêtes reliant le nœud à ses fils :
...
2. Immédiatement afficher un message invitant l'utilisateur à appuyer
   sur \keyenter.
3. Attendre que l'utilisateur appuie sur \keyenter. [fn:read_line]
...
Le tableau ~Sys.argv : string array~ contient les paramètres du
programme. Le premier élément du tableau est le nom du programme
exécuté. Pour savoir si le programme a reçu un paramètre
supplémentaire, vous pouvez donc vérifier que la longueur du tableau
est supérieure ou égale à 2, et le cas échéant, accéder à ce paramètre
par la valeur de ~Sys.argv.(1)~.
...
Des opérations du programme peuvent échouer, notamment :
- La lecture du fichier lexique, si vous n'exécutez pas la commande
  depuis la racine du projet.
- La construction de la grille, si le paramètre ne comporte pas un
  nombre carré de lettres.

Dans ces deux cas, la fonction correspondante renvoie ~None~. Le
programme devra alors terminer prématurément en affichant un message
décrivant l'erreur rencontrée. Pour terminer le programme OCaml, vous
pourrez écrire ~exit 1~.
